...
clear
echo
tte -i ~/.local/share/myarchy/logo.txt --canvas-width 0 --anchor-text c --frame-rate 920 laseretch
echo
...
if sudo test -f /etc/sudoers.d/99-myarchy-installer; then
  sudo rm -f /etc/sudoers.d/99-myarchy-installer &>/dev/null
fi
