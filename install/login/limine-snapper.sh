if command -v limine &>/dev/null; then
  ...
  sudo tee /etc/mkinitcpio.conf.d/myarchy_hooks.conf <<EOF >/dev/null
...
#timeout: 3
default_entry: 2
interface_branding: Myarchy Bootloader
...
#
#   uki_file=$(find /boot/EFI/Linux/ -name "myarchy*.efi" -printf "%f\n" 2>/dev/null | head -1)
#
#   if [[ -n "$uki_file" ]]; then
