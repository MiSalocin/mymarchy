...
# Users can still opt-in to Cloudflare DNS using: myarchy-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-myarchy-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-myarchy-dns.network
  ...
  echo "To use Cloudflare DNS, run: myarchy-setup-dns Cloudflare"
fi