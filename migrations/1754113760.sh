...
if ! grep -q 'on_unlock_cmd *= *myarchy-restart-waybar' ~/.config/hypr/hypridle.conf; then
  sed -i \
    '/^    on_unlock_cmd = pkill -SIGUSR2 waybar[[:space:]]*# prevent stacking of waybar when waking$/c\
    on_unlock_cmd = myarchy-restart-waybar  # prevent stacking of waybar when waking' \
