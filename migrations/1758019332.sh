...
myarchy-refresh-config uwsm/default
myarchy-refresh-config uwsm/env
myarchy-state set reboot-required
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, myarchy-launch-editor' ~/.config/hypr/bindings.conf
fi
