echo -e "Offer new Myarchy hotkeys\n"
...
if gum confirm "Switch to new hotkeys?"; then
  ...
  sed -i 's|source = ~/.local/share/myarchy/default/hypr/bindings/tiling\.conf|source = ~/.local/share/myarchy/default/hypr/bindings/clipboard.conf\
source = ~/.local/share/myarchy/default/hypr/bindings/tiling-v2.conf|' ~/.config/hypr/hyprland.conf
fi
