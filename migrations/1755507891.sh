...
echo "Migrating to use myarchy-launch-webapp and myarchy-launch-browser"
for desktop_file in ~/.local/share/applications/*.desktop; do
  if grep -q 'Exec=chromium --new-window --ozone-platform=wayland --app=' "$desktop_file"; then
    ...
    if [[ -n "$url" ]]; then
      sed -i "s|^Exec=.*|Exec=myarchy-launch-webapp \"$url\"|" "$desktop_file"
    fi
  fi
done
...
if [ -f "$HYPR_BINDINGS_FILE" ]; then
  sed -i 's/\$browser =.*chromium.*$/\$browser = myarchy-launch-browser/' "$HYPR_BINDINGS_FILE"
  sed -i 's/\$webapp="/myarchy-launch-webapp "/g' "$HYPR_BINDINGS_FILE"
